...
#include <tna.p4>
#endif
#include "common/headers_forwarding.p4"
...
#define FLOW_TABLE_LENGTH 65535
...
header bridged_metadata_t {
    ...
    pkt_type_t pkt_type;
    bit<32> chaff_remaining;
    @flexible bit<1> do_egr_mirroring;
    @flexible MirrorId_t egr_mir_ses;
}
...
struct header_t {
    ...
    tcp_h inner_tcp;
    udp_h inner_udp;
    // latency_h latency_head;
    // Add more headers here.
}


#include "common/parde.p4"
#include "srv6.p4"
...
//-----------------------------------------------------------------------------
// Chaff insert logic
// - Mirror packet if the packet count of one flow reach the given number
//-----------------------------------------------------------------------------

    CRCPolynomial<bit<16>>(16w0x1021,
                            true,
                            false,
                            false,
                            16w0x0000,
    ...
    Hash<bit<16>>(HashAlgorithm_t.CUSTOM, poly_id) hash_id;

    action get_id_action() {
        ig_md.flow_key = hash_id.get(
    ...
    table get_id_table {
        size = 1;
        actions = { get_id_action; }
        const default_action = get_id_action;
    }

    Register<bit<32>, bit<16>>(FLOW_TABLE_LENGTH) burst_dirc;

    RegisterAction<bit<32>, bit<16>, bit<8>>(burst_dirc) change_dirc = {
    ...
    Register<bit<32>, bit<16>>(FLOW_TABLE_LENGTH) flow_cnt;
    
    RegisterAction<bit<32>, bit<16>, bit<32>>(flow_cnt) count_incr_action = {
        void apply(inout bit<32> value, out bit<32> read_value) {
            read_value = value;
            if(ig_md.is_change == 1) {
                value = value + 1;
            }
        }
    };

    action set_interest_normal() {
        ig_md.is_interest = 1;
        ig_md.is_change = change_dirc.execute(ig_md.flow_key);
    }

    table select_flow_table { 
        key = {
            ig_md.flow_key          : exact;
        }
        actions = {
            set_interest_normal;
            NoAction;
    ...
    action incr_action(){
        ig_md.count = count_incr_action.execute(ig_md.flow_key);
    }

    table count_incr_table {
        key = {
            ig_md.is_change       : exact;
        }
        actions = {
            incr_action;
            NoAction;
    ...
    action get_chaff_count(bit<32> chaff_count, MirrorId_t mirror_session) {
        ig_md.chaff_remaining = chaff_count;
        ig_md.mirror_session = mirror_session;
    }

    table chaff_generation_table {
        key = {
            ig_md.flow_key: exact;
            ig_md.count: exact;
            ig_md.is_interest : exact;
        }
        actions = {
            get_chaff_count;
            NoAction;
    ...
    action set_normal_packet() {
        hdr.bridged_md.setValid();
        hdr.bridged_md.pkt_type = PKT_TYPE_NORMAL;
    }

    apply {
        // hdr.latency_head.ingress_global = ig_prsr_md.global_tstamp;
        // hdr.latency_head.ingress_mac = ig_intr_md.ingress_mac_tstamp;
        get_id_table.apply();
        select_flow_table.apply();
        count_incr_table.apply();

        if(ig_md.is_interest == 1) {
            chaff_generation_table.apply();
        }
...
control SwitchEgress(
    ...
    apply {
        ...
        // hdr.latency_head.enqueue = eg_intr_md.enq_tstamp;
        // hdr.latency_head.dequeue_delta = eg_intr_md.deq_timedelta;
        // hdr.latency_head.egress_global = eg_intr_from_prsr.global_tstamp;
        if(eg_md.is_chaff == 1) {       
            if(eg_md.chaff_remaining > 1){
                eg_md.chaff_remaining = eg_md.chaff_remaining - 1;
                eg_intr_md_for_dprsr.mirror_type = MIRROR_TYPE_E2E;
            }
        }
        else if(eg_md.chaff_remaining > 0) {
            eg_intr_md_for_dprsr.mirror_type = MIRROR_TYPE_E2E;
            eg_md.chaff_remaining = hdr.bridged_md.chaff_remaining;
        }
